...
def prepare_tinyimagenet_data():
    ...
    return poison_test_loader, poison_baseline_loader
...
def method_comparison(test_loader, baseline_loader, model, model_name, method_name_list, num_explain, t_mean, t_std, classes_name):
    ...
    print(model_name)
    counter = 0
    for batch_idx, ((image, target), (baseline, targets2)) in enumerate(zip(test_loader, baseline_loader)):
    ...
    coverage_rate /= counter
    random_coverage_rate /= counter
...
if __name__ == "__main__":
    ...
    # load data, models
    poison_test_loader, poison_baseline_loader = prepare_tinyimagenet_data()  # batch_size=1
    model_path_list = ['...']
    model_name_list = ['...']
    ...
    for idx, model in enumerate(tinyimagenet_model_list):
        method_comparison(poison_test_loader, poison_baseline_loader, model,
