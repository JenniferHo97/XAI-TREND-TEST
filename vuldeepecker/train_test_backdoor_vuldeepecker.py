...
def preprocess_data(data, w2v):
    data, labels, types = embedding_data(data, w2v)
    data = padding(data, types)
    return data, labels
...
def train_model(data_file_path_format: str,
    ...
    if allow_cache:
        for dirpath, dirnames, filenames in os.walk(
            ...
            for filename in filenames:
                matchObj = re.match(r'clear_vuldeepecker_model', filename)
                if matchObj:
                    file_exist_flag = 1
                    break
        if file_exist_flag:
            print('clear model already exists')
    ...
    if file_exist_flag == 0:
        ...
        clear_vuldeepecker_model = VULDEEPECKER_BILSTM(embedding_dim).to(
            device)
        clear_model_path_format = './models/clear_models/clear_vuldeepecker_model--epoch_{}--loss_{:.4f}--precision_{:.4f}--recall_{:.4f}--acc:{:.4f}.pth'
        clear_vuldeepecker_model = vuldeepecker_train_process(
            clear_vuldeepecker_model,
            train_loader,
            test_loader,
            epochs,
            criterion,
            clear_model_path_format,
            attn_flag=0)
    ...
    if allow_cache:
        ...
        for dirpath, dirnames, filenames in os.walk(
            ...
            for filename in filenames:
                matchObj = re.match(r'clear_vuldeepecker_attn_model', filename)
                if matchObj:
                    file_exist_flag = 1
                    break
        if file_exist_flag:
            print('clear attn model already exists')
    if file_exist_flag == 0:
        print('training clear attn model...')
        train_loader, test_loader = prepare_torch_dataloader(
            clear_train_data, clear_train_labels, clear_test_data,
            clear_test_labels, batch_size, num_workers)
        clear_vuldeepecker_attn_model = VULDEEPECKER_BILSTM_ATTN(
            embedding_dim).to(device)
        clear_attn_model_path_format = './models/clear_models/clear_vuldeepecker_attn_model--epoch_{}--loss_{:.4f}--precision_{:.4f}--recall_{:.4f}--acc:{:.4f}.pth'
        clear_vuldeepecker_attn_model = vuldeepecker_train_process(
            clear_vuldeepecker_attn_model,
            train_loader,
            test_loader,
            epochs,
            criterion,
            clear_attn_model_path_format,
            attn_flag=1)
    ...
    if file_exist_flag == 0:
        ...
        for ratio in ratio_list:
            ...
            train_loader, test_loader = prepare_torch_dataloader(
                ...
                clear_test_labels, batch_size, num_workers)
            str_vuldeepecker_model = VULDEEPECKER_BILSTM(embedding_dim).to(
                device)
            str_model_path_format = './models/backdoor_models/str_vuldeepecker_model--ratio_{}--epoch_{}--loss_{:.4f}--precision_{:.4f}--recall_{:.4f}--acc:{:.4f}.pth'
    ...
    if allow_cache:
        ...
        for dirpath, dirnames, filenames in os.walk(
            ...
            for filename in filenames:
                matchObj = re.match(r'str_vuldeepecker_attn_model', filename)
                if matchObj:
                    file_exist_flag = 1
                    break
        if file_exist_flag:
            print('str backdoor attn model already exists')
    ...
    if file_exist_flag == 0:
        ...
        for ratio in ratio_list:
            ...
            str_vuldeepecker_attn_model = VULDEEPECKER_BILSTM_ATTN(
                embedding_dim).to(device)
            str_attn_model_path_format = './models/backdoor_models/str_vuldeepecker_attn_model--ratio_{}--epoch_{}--loss_{:.4f}--precision_{:.4f}--recall_{:.4f}--acc:{:.4f}.pth'
