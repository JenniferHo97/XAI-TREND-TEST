...
def test(epoch, test_loader, backdoor_test_loader, model):
    global best_acc
    model.eval()
    test_loss = 0
    correct = 0
    ...
    torch.save(
        model,
        "./models/backdoor_models/damd_CNN--epoch_{}--loss_{:.4f}--acc:{:.4f}--backdoor_acc:{:.4f}.pth".
        format(epoch, test_loss, acc, backdoor_acc),
    )
...
if __name__ == "__main__":
    ...
    for epoch in range(0, epochs):
        ...
        test(epoch, test_loader, test_backdoor_loader, model)
